import { getAllSensorRules, getAllSensors, Sensor } from '@/lib/db/sensor-rules';
...
async function createAutomatedZone(rule: any, reading: SensorReading): Promise<string | null> {
    try {
        const zones = await getAllZones();
        
        // Check if zone already exists from this rule (prevent duplicates)
        const existingZone = zones.find((z: any) => 
