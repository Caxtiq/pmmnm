...
import { faShareAlt, faTimes, faCopy, faCheck } from '@fortawesome/free-solid-svg-icons';
...
export default function ShareButton({ reportId, description, type, severity }: ShareButtonProps) {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Chia sẻ báo cáo</h3>
          <button 
            onClick={() => setIsOpen(false)}
            className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
          >
        ...
        <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
          <p className="text-xs text-gray-500 dark:text-gray-400 break-all">{shareUrl}</p>
        </div>
      </div>
    </div>
  );
